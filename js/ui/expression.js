   ...
   $.fn.extend({
     ...
     my = {
         ...
         replaceSmile: function(str) {
				...
				str = str.replace(/\[em_(.*?)_([0-9]*)\]/g, '<img src="' + path.localhostImg + '/face/i_f_$2.gif" title="$1" border="0" style="float:none;margin:0;" />');
				return str;
			},
	   ...
	   replaceForSmile: function(str) {
				var smileContent = ['微笑', '大笑', '调皮', '惊讶', '耍酷', '发火', '害羞', '汗水', '大哭', '自满', '鄙视', '困', '夸奖', '晕倒', '疑问', '媒婆', '狂吐', '青蛙', '发愁', '亲吻', '挑衅', '爱心', '心碎', '玫瑰', '礼物', '难过', '奸笑', '可爱', '得意', '呲牙', '暴汗', '楚楚可怜', '睡觉', '哭', '生气', '震惊', '口水', '彩虹', '夜空', '太阳', '钱钱', '灯泡', '咖啡', '蛋糕', '音乐', '爱', '胜利', '赞', '踩', 'OK'];
				for (var i = 0; i < smileContent.length; i++) {
					str = str.replace(new RegExp('<img src="' + path.localhostImg + '/face/i_f_' + (i + 1) + '.gif" title="' + smileContent[i] + '" border="0" style="float:none;margin:0;">', "gm"), "[" + smileContent[i] + "]");
				}
				return str;
		},

     //表情插件
       showFace: function(jqele,jqinput) {
		            var id = 'facebox';
					var path = publicPatch.imgPath;
					...
					jqele.unbind().bind("mousedown", function() {
						var assign = jqinput;
						var strFace, labFace;
						if ($('#facebox').length <= 0) {
							...
							for (var i = 1; i <= SmileyInfor.length; i++) {
								...
								strFace += '<td bgcolor="#FFFFFF"><img title="' + SmileyInfor[i - 1] + '" src="' + path + '/face/i_f_' + i + '.gif" /></td>';
								if (i % 10 == 0) {
									strFace += '</tr>';
								}
							}
						...
						$(".dodoFace img").bind({
							click: function(e) {
								...
								assign.setCaret();
								assign.insertAtCaret("[" + SmileyInfor[num] + "]");
								$("#facebox").remove();
								e.stopPropagation();
							}
						});
